...
def h2theta_BrooksCorey(h,Par):
    thetar, eps, hb, lamb = Par    
    
    theta = thetar + (eps - thetar) *(hb/h)**lamb
    thetas = eps
...
def h2theta_Campbell(h,Par):
    eps, hb, b = Par    
    theta = eps * (hb/h)**(1.0/b)
    thetas = eps
    theta[h>=hb] = thetas

    return theta

def h2theta_Kosugi(h,Par):
    thetar, thetas, hm, sigma = Par
    theta = thetar + 0.5*(thetas - thetar) * erfc(numpy.log(h/hm)/(numpy.sqrt(2.0)*sigma))
    
    return theta
    
def h2theta_Khlosi(h,Par):
    thetar, thetas, hm, sigma = Par
    h0 = -10e7
    theta = thetar * (1-numpy.log(abs(h))/numpy.log(abs(h0))) + 0.5*(thetas - thetar * (1-numpy.log(abs(h))/numpy.log(abs(h0))) ) * erfc(numpy.log(h/hm)/(numpy.sqrt(2.0)*sigma))
...
def theta2h_Campbell_ISBA(theta,par):
    h_sat,thetas,b = par
    h = h_sat * (theta/thetas)**(-b)
    h = GenericReplacement(h>h_sat,h,h_sat)
    return h

def h2theta_Campbell_ISBA(h,par):
    h_sat,thetas,b = par
    theta = thetas*(h/h_sat)**(1/-b)
    theta = GenericReplacement(h>h_sat,theta,thetas)
    return theta
...
def theta2K_Campbell_ISBA(theta,par):
    Ks,thetas,b = par
    K = Ks * (theta/thetas)**(2*b+3)
    K = GenericReplacement(theta>=thetas,K,Ks)
    return K

def h2K_Campbell_ISBA(h,par):
    Ks,h_sat,thetas,b = par
    theta = h2theta_Campbell_ISBA(h,[h_sat,thetas,b])
    K = theta2K_Campbell_ISBA(theta,[Ks,thetas,b])
    return K
...
def h2r_YoungLaplace(h):
    ...
    r = (2*surftens*numpy.cos(contactangle))/(-h*rho_w*g) # pore radius in meter

    return r

    
def r2h_YoungLaplace(r):
    ## r in meter
    surftens=0.0728 # N/m
    contactangle=0 # rad
    g=9.81 # m/s
    ...
    h = -(2*surftens*numpy.cos(contactangle))/(r*rho_w*g) # head in meter

    return h

    
    
def r2psd_Kosugi(r,Par):
    # r in meter
    thetar,thetas,r0,std=Par
    
    psd=(thetas-thetar)/(r * std * numpy.sqrt(2.0 * numpy.pi)) \
           * numpy.exp(-(numpy.log(r/r0))**2 / (2*std**2))

    return psd


def r2psd_VanGenuchten4(r,Par):
    print('NEEDS TO BE CHECKED')
    # r in meter
    thetar,thetas,alpha,n=Par
    alpha = alpha * 1e2 # 1/cm to 1/m
    m=1.0-1.0/n
    h = r2h_YoungLaplace(r) *1e-2 # cm

    psd=-(thetas-thetar)*(n*alpha**n*abs(h)**(n-1))*(-m)*(1+alpha**n*abs(h)**n)**(-m-1)
